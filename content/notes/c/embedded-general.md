...
{{% alert note %}}
**DEF.** When a register has to be modified, _only the bits that are needed should be changed._
```c
P1->DIR &= ~0x02; // change only bit 1 in the DIR register of port 1
```
{{% /alert %}}
...
Also, the voltage range should be considered and will affect correct communication. Some common voltage levels are reported in the following figure.
...
{{% alert warning %}}
There are $3.3\,\text{V}$ devices that can accept $5\,\text{V}$ signals on some pins (and so they are called **5V-tolerant**). Unfortunately, this is generally not true, _so be careful to control the communicating logic used between 2 connected devices._
