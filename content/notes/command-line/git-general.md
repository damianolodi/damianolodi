...
- **Staging area** &rarr; it is a file maintained by git that contains all the changes that have been marked to be included in the next commit. Each file can be in one of three different states: _modified_, _staged_ or _committed_.
- **Commit** &rarr; a snapshot of the current status of the project.
- **HEAD** &rarr; it represents the currently checked-out snapshot of the project. One can think to it as a pointer to the current branch.
...
- **Branch** &rarr; it represents an independent line of development in a project. The default branch that git creates when the project is initialized is called _master._
...
The `--global` argument states to set these configurations as default for each directory in the systems. One can change those parameters for a single project if needed (just by omitting the `--global` argument).
...
- `git log` &rarr; return information on the last few commits.
  - `git log -<limit>` &rarr; limit the number of commits to the specified number.
  - `git log --stat` &rarr; return information on the last few commits adding more statistics (how many files and lines where changed).
	- `git log -p` &rarr; (`p` stands for _patch_) return informations on the last few commits and all the changes made in each committed file.
	- `git log -p branch_name` &rarr; return information on the changes between the current working tree and the last commit of the provided branch.
	- `git log --graph —oneline --all` &rarr; print only the first line of the commit messages and plot the log to be similar to a git graph, for a better visual explanation of the git history. Use `--all` if the complete history is needed.
- `git diff` &rarr; show differences between the unstaged files in the current working tree and the HEAD commit. It can be useful to explore when adding a big amount of code in a single commit, to remember exactly what was changed.
  - `git diff <file>` &rarr; show the differences concerning only the provided file.
- `git show commit_ID` &rarr; show differences between the current working tree and the commit provided.

### Committing

- `git add <file_name>` &rarr; add the file to the staging area.
  - `git add .` &rarr; stage all files
- `git rm file_name` &rarr; the file is removed from the directory and the change is automatically staged for the next commit.
...
- `git commit` &rarr; commit the files added in the staging area. It opens an editor in which one can insert a detailed commit message.
	- `git commit -m "message"` &rarr; create a commit with a short message (it doesn't open the default git editor).
...
2. Empty line.
3. A more detailed explanation of the changes, divided in paragraphs.
  - _Try to keep each line under 72 characters in lenght_ (because the command `git log` does not do any line wrapping).
  - This explanation can reference bugs or issues that were corrected, or include links to more information when relevant.
...
- [More advices for commit messages](https://thoughtbot.com/blog/5-useful-tips-for-a-better-commit-message)
